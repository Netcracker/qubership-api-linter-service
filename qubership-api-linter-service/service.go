...
func main() {
	...
	r.HandleFunc("/api/v1/packages/{packageId}/versions/{version}/validation", authenticator.Secure(validationController.ValidateVersion)).Methods(http.MethodPost)
	r.HandleFunc("/api/v1/bulkValidation", authenticator.Secure(validationController.StartBulkValidation)).Methods(http.MethodPost)
	r.HandleFunc("/api/v1/bulkValidation/{jobId}", authenticator.Secure(validationController.GetBulkValidationStatus)).Methods(http.MethodGet)

	// Validation result
	r.HandleFunc("/api/v1/packages/{packageId}/versions/{version}/validation/summary", authenticator.Secure(validationResultController.GetValidationSummaryForVersion_deprecated)).Methods(http.MethodGet)
	r.HandleFunc("/api/v2/packages/{packageId}/versions/{version}/validation/summary", authenticator.Secure(validationResultController.GetValidationSummaryForVersion)).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/packages/{packageId}/versions/{version}/validation/documents/{slug}/details", authenticator.Secure(validationResultController.GetValidationResultForDocument_deprecated)).Methods(http.MethodGet)
	r.HandleFunc("/api/v2/packages/{packageId}/versions/{version}/validation/documents/{slug}/details", authenticator.Secure(validationResultController.GetValidationResultForDocument)).Methods(http.MethodGet)

	// Scoring
	r.HandleFunc("/api/v1/packages/{packageId}/versions/{version}/scoring", authenticator.Secure(scoringController.GetScoringForVersion)).Methods(http.MethodGet)

	// Linters
	r.HandleFunc("/api/v1/linters", authenticator.Secure(linterController.ListLinters)).Methods(http.MethodGet)

	// Ruleset management
	r.HandleFunc("/api/v1/rulesets", authenticator.Secure(rulesetController.CreateRuleset)).Methods(http.MethodPost)
	r.HandleFunc("/api/v1/rulesets/{ruleset_id}/activation", authenticator.Secure(rulesetController.ActivateRuleset)).Methods(http.MethodPost)
	r.HandleFunc("/api/v1/rulesets", authenticator.Secure(rulesetController.ListRulesets)).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/rulesets/{ruleset_id}", authenticator.Secure(rulesetController.GetRuleset)).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/rulesets/{ruleset_id}/data", authenticator.NoSecure(rulesetController.GetRulesetData)).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/rulesets/{ruleset_id}/activation", authenticator.Secure(rulesetController.GetRulesetActivationHistory)).Methods(http.MethodGet)
	r.HandleFunc("/api/v1/rulesets/{ruleset_id}", authenticator.Secure(rulesetController.DeleteRuleset)).Methods(http.MethodDelete)
